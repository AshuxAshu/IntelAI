...
The arms are the official Menagerie SO-101 MJCF, mounted side by side on the
operator-facing front edge: the opposing-end mounts tried first exceeded the
real SO-101 reach (0.48 m measured against 0.62 m to the shared-zone far
corner), which made both the shared zone and the drawer unreachable. Arm A
takes the plate / drawer / cutlery side, arm B the mug side.
"""
...
# The ee site sits 8 mm further along the fingers, which retracted the
# reachable envelope by the same 8 mm and cost the lens its two far corners at
# y = -0.16; re-measured, the dual-reach lens now ends at y = -0.18.
...
PLACEMATS = {
    ...
    # Cutlery has its own fork and spoon settings beside the plate: the fork
    # west of the plate setting, the spoon
